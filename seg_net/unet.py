...
def densenet_matte(filters, classes, kernel_size = 3, stride = 1, use_batchnorm=False, sep_conv = True, n_layers = 3, skip = None):
    ...
    def layer(input_tensor):
        ...
        if not sep_conv:
	        x = keras.layers.Conv2D(classes, (3,3), padding='same', name='densenet_matt_final_conv')(x)
        else:
	        x = keras.layers.SeparableConv2D(classes, (3,3), padding='same', name='densenet_matt_final_conv')(x)
...
def Unet(pretrained = None,
    ...
    if not sep_conv:
    	...
    	#x = SepConv_BN(classes, 'final_conv', stride=1, kernel_size=3, rate=1,
        #                   use_batchnorm=True, depth_activation=True, epsilon=1e-3)(x)
