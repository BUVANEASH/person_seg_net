...
def DecoderBlock(filters, stage, kernel_size=3, upsample=(256,256), 
    ...
    def layer(input_tensor):
        ...
        if not sep_conv:  
            x = ConvRelu(input_filters // 4, kernel_size, use_batchnorm=use_batchnorm,
        ...
        if not sep_conv:
            ...
            x = SepConv_BN(output_filters, layer_name+ '2', stride=1, kernel_size=3, rate=1,
                           use_batchnorm=True, depth_activation=True, epsilon=1e-3)(x)
...
def Linknet(pretrained = None,
    ...
    for i in range(n_upsample_blocks):
            ...
            x = DecoderBlock(decoder_filters[i], i, upsample=upsample,
    ...
    if not sep_conv:
        ...
        x = SepConv_BN(classes, 'final', stride=1, kernel_size=3, rate=1,
