...
def densenet_matte(filters, kernel_size = 3, stride = 1, use_batchnorm=False, sep_conv = True, n_layers = 3):
...
def Upsample2D_Block(filters, stage, kernel_size=3, upsample=(256,256), 
    ...
    def layer(input_tensor):
        ...
        if not sep_conv:
            ...
            layer_name = 'upsample{}_'.format(stage)
...
def FCN8(pretrained = None,
    ...
    x = SepConv_BN(4096, 'fc1', stride=1, kernel_size=7, rate=2,
                   use_batchnorm=False, depth_activation=True, epsilon=1e-3)(x)
    x = keras.layers.Dropout(0.5)(x)
    x = SepConv_BN(4096, 'fc2', stride=1, kernel_size=7, rate=2,
                   use_batchnorm=False, depth_activation=True, epsilon=1e-3)(x)
    x = keras.layers.Dropout(0.5)(x)
    
    x = keras.layers.SeparableConv2D(classes, (1, 1), kernel_initializer='he_normal', activation='linear', padding='valid', strides=(1, 1))(x)
    
    skip1 = SepConv_BN(classes, 'skip1', stride=1, kernel_size=1, rate=1,
                       use_batchnorm=False, depth_activation=True, epsilon=1e-3)(encoder.layers[skip_connection_idx[0]].output)
    x = Upsample2D_Block(classes, '1', upsample=(input_shape[0]//16,input_shape[1]//16),
                         skip=skip1, use_batchnorm=False, sep_conv = True)(x)
    
    skip2 = SepConv_BN(classes, 'skip2', stride=1, kernel_size=1, rate=1,
                       use_batchnorm=False, depth_activation=True, epsilon=1e-3)(encoder.layers[skip_connection_idx[1]].output)
    x = Upsample2D_Block(classes, '2', upsample=(input_shape[0]//8,input_shape[1]//8),
                         skip=skip2, use_batchnorm=False, sep_conv = True)(x)
    
    x = Upsample2D_Block(classes, 'final_conv', upsample=(input_shape[0],input_shape[1]),
                         skip=None, use_batchnorm=False, sep_conv = True)(x)
    
    if densenet_matting_layers:
        dense_matt_skip = x
        x = keras.layers.Add()([x, img_input])
        x = densenet_matte(64, kernel_size = 3, stride = 1, 
                           use_batchnorm=False, sep_conv = matt_sep_conv, n_layers = densenet_matting_layers)(x)
        if not sep_conv:
            x = keras.layers.Conv2D(classes, kernel_size = (1,1) , strides = (1,1),
                                             padding='same', name='densenet_matt_final_conv')(x)
        else:
            x = SepConv_BN(classes, 'densenet_matt_final_conv', stride=1, kernel_size=1, rate=1,
                       use_batchnorm=True, depth_activation=True, epsilon=1e-3)(x)
        x = keras.layers.Add()([x, dense_matt_skip])
