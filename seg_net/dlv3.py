...
def Deeplabv3(weights='pascal_voc', input_tensor=None, input_shape=(512, 512, 3), classes=21, 
    ...
    if backbone == 'xception':
        ...
        x = layers.ReLU(6.0, name='Conv_Relu6')(x)
    ...
    b4 = Lambda(lambda x: K.tf.image.resize(x, (int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))),
    ...
    if backbone == 'xception' or mn_decoder:
        ...
        x = Lambda(lambda xx: K.tf.image.resize(xx,
                                                (int(np.ceil(input_shape[0] / 4)),int(np.ceil(input_shape[1] / 4))),
                                                method=K.tf.image.ResizeMethod.BILINEAR,
